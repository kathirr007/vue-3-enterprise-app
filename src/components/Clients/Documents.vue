...
const canDoActions = inject('canDoActions', false);
...
const {
  ...
  tabRef,
  nestedTabRef
} = useSteps(`${isPortalUser.value ? 'portal-documents' : 'admin-clients-id'}`);
const {
  uploadFileRef,
  uploadDialog,
  onUpload,
...
const canCreateFolderCheck = computed(() => {
  return (
    !(currentFolderData.value as DocumentFolder)?.isExtractionFolder
    && !(currentFolderData.value as DocumentFolder)?.isSmartFolder
  );
});
...
async function handleOnUpload(data: UploadFilesPayload) {
  await onUpload({ payload: data, isGalleryUpload: false });
  queryClient.invalidateQueries('files-list');
  queryClient.invalidateQueries('folders-list');
  queryClient.invalidateQueries('search-docs');
  queryClient.invalidateQueries('document-limit');
...
function handleFolderData(data: DocumentFolder | DocumentFolder[]) {
  /* if (Array.isArray(data)) {
    data.forEach((folder: any) => {
      canCreateFolderCheck.value =
        !folder?.isSmartFolder || !folder?.isExtractionFolder;
    });
  } else {
    canCreateFolderCheck.value =
      !data?.isSmartFolder || !data?.isExtractionFolder || false;
  } */
...
<template>
        ...
        <div class="my-4 md:m-0 ml-auto">
          <template v-if="isPortalUser && !!folderId && canCreateFolderCheck">
            ...
            >
              <Button
                ...
                :disabled="
                  (!(currentFolderData as DocumentFolder)?.clientWritable) || (documentResource.limit && (documentResource.usage >= documentResource.limit))
                "
                @click="toggle"
              />
            </span>
          ...
          <span
            v-else-if="
              !isPortalUser
                && (canAccessAllMenu
                  || (clientDetails as unknown as Client)?.relationshipManager
                    ?.id === currentUser.id)
                && canCreateFolderCheck
            "
          ...
          >
            <Button
              ...
              :disabled="
                !canDoActions
                  || (currentFolderData as DocumentFolder)?.isExtractionFolder || (documentResource.limit && (documentResource.usage >= documentResource.limit))
              "
              @click="toggle"
            />
          </span>
  ...
  <Dialog
    ...
    :style="{ width: '35vw' }"
  >
    <ClientsDocumentCreateFolder @success="handleCreateSuccess" />
  </Dialog>
  <Dialog
