...
const searchedFileFolders = ref<SearchedReponse>();
...
const listViewData = computed<any[]>(() => {
  ...
  if (staticSearchText.value && staticSearchText.value?.length >= 3) {
    if (searchedFileFolders.value) {
      allDocuments = [
        ...(searchedFileFolders.value?.folders as DocumentFolder[]).filter(
          (item: DocumentFolder) =>
...
function filterFolderMenuItems(folder: DocumentFolder) {
  const clientReadable = ['Open', 'Download'];
...
const { isFetching: searchIsFetching } = useQuery(
  ['search-gallery-docs', staticSearchText],
  useDebounceFn(async () => {
    if ((staticSearchText.value as string)?.length >= 3) {
      searchedFileFolders.value = await searchFilesAndFolders({
        id: 'sampleId',
        mode: 'all',
...
const { data: files, isFetching: fetchingFiles } = useQuery(
  ['gallery-files', ...queryKeys, selectedTagFilter],
  () => {
    return getFiles({
      id: 'sampleId',
...
const { mutateAsync: deleteDoc, isLoading: deletingFile } = useMutation(
  ['gallery-document-remove'],
  () => {
...
<template>
  <div class="flex justify-content-between mb-4 align-items-center">
    ...
    >
      <div v-if="mode === 'grid'" class="flex justify-content-end px-2">
  ...
  <transition-group name="slide-up">
              ...
              <div class="p-2">
                <div class="flex align-items-center">
                  ...
                  <div class="ml-auto">
                    <Button
                      :icon="
                        deletingFile && dataToModify?.id === file.id
    ...
    <div v-if="mode === 'list'" key="list" class="mt-4">
      <DataTable
        ...
        breakpoint="768px"
        :loading="
          loadingFolders || fetchingFolders || fetchingFiles || searchIsFetching
        "
        :paginator="false"
      >
        <template #header>
          <div class="flex justify-content-end">
            ...
            <div>
              <!-- <ClientsBulkAction
                :clientId="clientId || currentUser.client.id"
          ...
          <!-- <div class="my-2" v-if="hideFilter">
        ...
        <!-- <Column
          v-if="!isPortalUser || folderDetails?.clientWritable"
        ...
        >
          <template #body="{ data }">
            <div
              ...
              tabindex="0"
              @click.prevent="navigateToItem(data)"
        ...
        <Column header="Actions">
          <template #body="{ data }">
            <Button
              :icon="
                deletingFile && dataToModify?.id === data.id
                  ? 'pi pi-spin pi-spinner'
                  : 'pi pi-ellipsis-v'
              "
