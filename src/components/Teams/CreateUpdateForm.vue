...
import { UpdateTeamMemberPayloadSchema } from '@/types/teams.type';
...
const mobileHelpText = ref<any>();
const phoneHelpText = ref<any>();
const formRef = ref<SchemaFormRef>();
const telInputMobileRef = ref<Component>();
const telInputPhoneRef = ref<Component>();
const formKey = ref(0);
  ...
  return {
    fields: [
      ...
      },
      {
        ...
        optionValue: 'id',
        filter: false,
        disabled: props.user?.isOwner || userId.value === currentUser.value?.id,
        hide: isPortalUser.value,
        options: roles.value
      },
      {
    ...
    validationSchema: UpdateTeamMemberPayloadSchema,
...
const disableSubmit = computed(() => {
  // return !formRef.value?.meta?.valid || !(telInputPhoneRef.value as any)?.meta?.valid;
  return !formRef.value?.meta?.valid;
});

function setTelInputValue(val: any, fieldName: string) {
  formRef.value?.setFieldValue(fieldName, val);
}
...
const {
  ...
  (payload: CreateUserPayload | UpdateTeamMemberPayload) => {
    if (props.user) {
      return useUserUpdateDetails(
        props.user.id,
        payload as UpdateTeamMemberPayload,
        isPortalUser.value
      );
    }
...
async function onSubmit(values: Record<string, any>) {
  const payload: Partial<
    (CreateUserPayload | UpdateTeamMemberPayload) & { salary: string }
  > = { ...values, salary: values.salary?.toString() };
  if ((values as UpdateTeamMemberPayload).phone === '') {
    (payload as UpdateTeamMemberPayload).phone = null;
  }
  if ((values as UpdateTeamMemberPayload).mobile === '') {
    (payload as UpdateTeamMemberPayload).mobile = null;
  }
  await createUpdateTeamMember(
    payload as unknown as (CreateUserPayload | UpdateTeamMemberPayload) & {
      salary: string;
    }
  );
}

function toggleMobileHelp(event: any) {
  mobileHelpText.value?.toggle(event);
}
function togglePhoneHelp(event: any) {
  phoneHelpText.value?.toggle(event);
}

watchEffect(() => {
  if (props.user) {
    formRef.value?.setFieldValue('country', props.user?.country as string);
  }
});
</script>
