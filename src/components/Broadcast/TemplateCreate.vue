...
async function handleBasic(values: CreateBroadcastTemplate) {
  if (values.id) {
    await updateBroadcastTemplate({
      payload: {
        name: values.name,
        description: values.description,
        isInternal: broadcastTo === 'team',
...
function handleBack(step: 'basic' | 'message') {
  if (step === 'basic') {
    router.push({
...
<template>
  <div :class="currentStep !== 'message' ? 'lg:w-8 xl:w-6 mx-auto' : ''">
    ...
    />
    <h2 class="text-2xl">
      <span v-if="currentStep === 'basic'">{{ revisit ? 'Update/View' : 'Add' }} Basic Details</span>
